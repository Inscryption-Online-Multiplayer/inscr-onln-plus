...
func _ready():
	...
	# Version
	get_node("../VersionLabel").text = "IMF: " + CardInfo.VERSION + "\nPlus: " + CardInfo.PLUS
	...
	# default pfp
	$InLobby/Rows/ProfilePic/Pic._select_int(GameOptions.options.plus.defaultPfp)
	$InLobby/Rows/ProfilePic/Preview.texture = load("res://gfx/portraits/" + $InLobby/Rows/ProfilePic/Pic.get_item_text(GameOptions.options.plus.defaultPfp) + ".png")

	# default room name and username
...
func populate_deck_list():
	...
	while fName != "":
		if not dTest.current_is_dir() and fName.ends_with(".deck"):
			...
			$"../Options/TabContainer/Plus/ScrollContainer/VBoxContainer/misc/deck/defaultDeck".add_item(fName.split(".deck")[0])
...
func update_lobby():
	...
	if GameOptions.options.plus.autoReady and not isReady:
		_on_LobbyReady_pressed()
...
func init_fight(go_first: int):
	...
	# turn the blocker on so quickhost don't work
	$Blocker.visible = true
...
func _input(_event):
	if Input.is_action_just_released("quickHost") and not $Blocker.visible:
		_on_HostBtn_pressed()
		_on_Host_pressed()
	elif Input.is_action_just_released("quickJoin") and not $Blocker.visible:
		_on_JoinBtn_pressed()
		_on_PasteButton_pressed()
		_on_Join_pressed()
